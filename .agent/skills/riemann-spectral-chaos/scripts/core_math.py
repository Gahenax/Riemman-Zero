...
def brent_root(
    f: Callable[[float], float], a: float, b: float, tol: float = 1e-12
) -> Dict[str, Any]:
...
def try_bracket_and_confirm(
    f: Callable[[float], float], t0: float, step: float, expand: int = 8
) -> Dict[str, Any]:
...
def n_riemann_von_mangoldt(t: float) -> float:
    ...
    if t < 10:
        return 0  # Formula not accurate for very small t
