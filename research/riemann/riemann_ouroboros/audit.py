    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            for k in [
                ...
                "max_flow_norm",
                "reducer_ks",
                "reducer_entropy_before",
                "reducer_entropy_after",
            ]:
                if k in row and row[k]:
                    try:
                        row[k] = float(row[k])
...
def compute_summary_by_type(finals: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for btype, blocks in by_type.items():
        ...
        summary[btype] = {
            ...
            "feature_means": {
                k: round(float(means[i]), 8) for i, k in enumerate(FEATURE_KEYS)
            },
            "feature_stds": {
                k: round(float(stds[i]), 8) for i, k in enumerate(FEATURE_KEYS)
            },
        }
