...
def main():
    ...
    all_r_poi = [
        gap_ratios(poisson_spacings(block_sp, seed=ref_seed + 10000 + i))
        for i in range(n_ref)
    ]
    ...
    for wid, window, spacings in available:
        ...
        gue_blocks = build_disjoint_blocks_from_spacings(
            np.array([]), ctrl_cfg["gue_blocks_per_iter"], block_sp, iter_seed + 1000, "gue"
        )
        poi_blocks = build_disjoint_blocks_from_spacings(
            np.array([]),
            ctrl_cfg["poisson_blocks_per_iter"],
            block_sp,
            iter_seed + 2000,
            "poisson",
        )
        ...
        print(
            f"  Blocks: {len(zeta_blocks)} zeta + "
            f"{len(gue_blocks)} gue + {len(poi_blocks)} poi",
            flush=True,
        )
        ...
        for bi, block in enumerate(all_blocks):
            ...
            if (bi + 1) % 25 == 0 or bi == len(all_blocks) - 1:
                print(
                    f"    blocks: {bi + 1}/{len(all_blocks)} rows={writer.total_rows}",
                    flush=True,
                )
    ...
    report = run_audit(
        merged_csv, report_json, outliers_csv, topk=cfg["audit"].get("topk_outliers", 40)
    )
