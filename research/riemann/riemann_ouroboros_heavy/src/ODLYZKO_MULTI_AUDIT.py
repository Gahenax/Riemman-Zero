...
def main():
    ...
    r_poi_ref = np.concatenate(
        [gap_ratios(poisson_spacings(BLOCK_LEN, seed=80000 + i)) for i in range(N_REF)]
    )
    ...
    zeta_blocks = build_disjoint_blocks_from_spacings(
        spacings, n_avail, BLOCK_LEN, 42, "zeta"
    )
    gue_blocks = build_disjoint_blocks_from_spacings(
        np.array([]), CTRL_BLOCKS, BLOCK_LEN, 80000, "gue"
    )
    poi_blocks = build_disjoint_blocks_from_spacings(
        np.array([]), CTRL_BLOCKS, BLOCK_LEN, 90000, "poisson"
    )
    all_blocks = zeta_blocks + gue_blocks + poi_blocks
    print(
        f"  Blocks: {len(zeta_blocks)} zeta + {len(gue_blocks)} gue + {len(poi_blocks)} poi",
        flush=True,
    )
