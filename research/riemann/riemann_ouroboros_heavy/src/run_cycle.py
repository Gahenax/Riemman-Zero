...
from metrics import gap_ratios
...
def main():
    ...
    log(
        f"Controls: gue={cfg['controls']['gue_blocks_per_iter']}, "
        f"poi={cfg['controls']['poisson_blocks_per_iter']}",
        log_fh,
    )
    log("=" * 72, log_fh)
    ...
    try:
        for iteration in range(max_iters):
            ...
            window = (
                zeta_cfg["windows"][iteration]
                if iteration < len(zeta_cfg["windows"])
                else None
            )
            ...
            if zeta_cfg["enable"] and window:
                ...
                log(
                    f"  Unfolded: {len(spacings)} spacings, mean={np.mean(spacings):.4f}",
                    log_fh,
                )
                ...
                zeta_blocks = build_disjoint_blocks_from_spacings(
                    spacings, n_zeta_blocks, block_len, seed=iter_seed, type_name="zeta"
                )
                ...
                with Pool(processes=pool_size) as pool:
                    for result_rows in pool.imap_unordered(_worker, worker_args):
                        ...
                        if processed % 20 == 0 or processed == len(all_blocks):
                            log(
                                f"    blocks: {processed}/{len(all_blocks)} "
                                f"rows={writer.total_rows}",
                                log_fh,
                            )
            ...
            log(
                f"  Iteration time: {iter_dt:.1f}s | "
                f"merged: {writer.file_size_mb:.2f} MB | "
                f"total rows: {writer.total_rows}",
                log_fh,
            )
            ...
            if rob.get("zeta_available"):
                log(
                    f"  ROBUSTNESS: passing={rob['passing_intensities']}/{rob['total_intensities']} "
                    f"robust={rob['robust']}",
                    log_fh,
                )
    ...
    # === Final conclusion ===
    print(f"\n{'=' * 72}", flush=True)
    print("OUROBOROS HEAVY — FINAL REPORT", flush=True)
    ...
    if os.path.exists(report_json):
        ...
        if rob.get("zeta_available"):
            ...
            for intname, iv in rob.get("intensities", {}).items():
                print(
                    f"    {intname}: vote_rate={iv['vote_rate']:.0%} "
                    f"({iv['gue_votes']}/{iv['n']})",
                    flush=True,
                )
        else:
            print("  No zeta blocks analyzed (controls only).", flush=True)
