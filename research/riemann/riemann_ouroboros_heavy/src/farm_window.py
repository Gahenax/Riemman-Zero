...
def get_zeros_imag(n_start: int, n_end: int, dps: int, cache_path: str) -> np.ndarray:
    ...
    print(
        f"  [DONE] {count} zeros in {total_time:.0f}s "
        f"({count / max(total_time, 0.01):.2f}/s)",
        flush=True,
    )
    return zeros
