...
def main():
    ...
    print(
        f"  Available blocks (disjoint, {BLOCK_LEN} each): {n_blocks_available}", flush=True
    )
    ...
    all_r_poi = [
        gap_ratios(poisson_spacings(BLOCK_LEN, seed=ref_seed + 10000 + i))
        for i in range(N_REF_BLOCKS)
    ]
    r_gue_ref = np.concatenate(all_r_gue)
    r_poi_ref = np.concatenate(all_r_poi)
    print(
        f"  GUE ref: {len(r_gue_ref):,} ratios (mean={np.mean(r_gue_ref):.4f})", flush=True
    )
    print(
        f"  POI ref: {len(r_poi_ref):,} ratios (mean={np.mean(r_poi_ref):.4f})", flush=True
    )
    ...
    print(
        f"  Total: {len(all_blocks)} x {len(INTENSITIES)} intensities "
        f"= {len(all_blocks) * len(INTENSITIES)} rows",
        flush=True,
    )
    ...
    for bi, block in enumerate(all_blocks):
        ...
        if done % 50 == 0 or done == len(all_blocks):
            print(
                f"    {done}/{len(all_blocks)} blocks ({writer.total_rows} rows)",
                flush=True,
            )
    ...
    print(
        f"  Merged CSV: {writer.file_size_mb:.2f} MB, {writer.total_rows} rows", flush=True
    )
    writer.close()
    ...
    if rob.get("zeta_available"):
        ...
        for intname, iv in rob.get("intensities", {}).items():
            ...
            print(
                f"    {intname}: vote_rate={iv['vote_rate']:.1%} "
                f"({iv['gue_votes']}/{iv['n']}) [{status}]",
                flush=True,
            )
            total_int += 1
            if iv["vote_rate"] > 0.6:
        ...
        print(f"\n{'=' * 70}", flush=True)
        if robust:
            print("  VERDICT: ZETA_ZEROS_FAVOR_GUE", flush=True)
            print(f"  Robust across {passing}/{total_int} intensities", flush=True)
            print("  100,000 Odlyzko zeros confirm GUE alignment", flush=True)
        else:
    ...
    print("  DISCLAIMER: Statistical observation != proof of RH.", flush=True)
