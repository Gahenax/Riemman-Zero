...
def process_block(
    ...
    # KS vs controls
    ks_gue = (
        float(_st.ks_2samp(r_reduced, r_gue_ref).statistic) if len(r_reduced) > 5 else 1.0
    )
    ks_poi = (
        float(_st.ks_2samp(r_reduced, r_poi_ref).statistic) if len(r_reduced) > 5 else 1.0
    )
