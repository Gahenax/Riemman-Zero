...
def main():
    ...
    ap.add_argument("--strict", action="store_true", help="Abort on validation anomalies")
    ap.add_argument(
        "--max_zeros", type=int, default=0, help="Limit number of zeros (0 = all)"
    )
    args = ap.parse_args()
    ...
    print(f"\n{'=' * 60}", flush=True)
    print("ODLYZKO RIEMANN PIPELINE", flush=True)
    print(f"  Table: {args.table}", flush=True)
    print(f"  Description: {table['description']}", flush=True)
    print(
        f"  Expected: ~{table['expected_count']:,} zeros ({table['size_mb']} MB)",
        flush=True,
    )
    print(f"{'=' * 60}\n", flush=True)
    ...
    print(
        f"  Range: [{vinfo.get('t_min', 0):.4f}, {vinfo.get('t_max', 0):.4f}]", flush=True
    )
    print(
        f"  Gaps: min={vinfo.get('min_gap', 0):.6f}, "
        f"max={vinfo.get('max_gap', 0):.4f}, "
        f"mean={vinfo.get('mean_gap', 0):.4f}",
        flush=True,
    )
    ...
    print("  Ready for OUROBOROS HEAVY pipeline", flush=True)
