...
def compute_summary(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for (btype, intensity), group in groups.items():
        ...
        r_means = [
            r.get("r_mean", 0) for r in group if isinstance(r.get("r_mean"), (int, float))
        ]
...
def compute_robustness(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for intensity, group in by_intensity.items():
        ...
        rate = gue / max(n, 1)
        intensity_votes[intensity] = {"n": n, "gue_votes": gue, "vote_rate": round(rate, 4)}
        if rate > 0.6:
            passing += 1
...
def compute_outliers(rows: List[Dict[str, Any]], topk: int = 40) -> List[Dict[str, Any]]:
...
def run_audit(
    merged_csv: str, report_json: str, outliers_csv: str, topk: int = 40
) -> Dict[str, Any]:
    """Full audit pipeline."""
