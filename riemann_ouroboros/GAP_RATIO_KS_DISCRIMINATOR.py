...
def build_reference_distribution(
    kind: str, n_blocks: int = 100, block_length: int = 4000, base_seed: int = 7000
) -> np.ndarray:
    """
...
def analyze_block(
    spacings: np.ndarray, ref_gue: np.ndarray, ref_poi: np.ndarray
) -> Dict[str, Any]:
    """
...
def main():
    ...
    ap.add_argument("--block_length", type=int, default=4000, help="Spacings per block")
    ap.add_argument(
        "--vote_threshold",
        type=float,
        default=0.80,
    ...
    print("+" + "=" * 68 + "+", flush=True)
    print(
        "|  GAP RATIO KS DISCRIMINATOR                                      |", flush=True
    )
    print(
        "|  r_i = min(s_i, s_{i+1}) / max(s_i, s_{i+1})                     |", flush=True
    )
    print(
        "|  System: GAHENAX / Antigravity Core                               |", flush=True
    )
    print("+" + "=" * 68 + "+", flush=True)
    print("", flush=True)
    ...
    ref_gue = build_reference_distribution(
        "gue",
        n_blocks=args.n_ref_blocks,
        block_length=args.block_length,
        base_seed=args.base_seed + 50000,
    )
    ref_poi = build_reference_distribution(
        "poisson",
        n_blocks=args.n_ref_blocks,
        block_length=args.block_length,
        base_seed=args.base_seed + 60000,
    )
    print(
        f"  GUE reference: {len(ref_gue)} ratios (mean={np.mean(ref_gue):.4f})", flush=True
    )
    print(
        f"  Poisson reference: {len(ref_poi)} ratios (mean={np.mean(ref_poi):.4f})",
        flush=True,
    )
    print("  Expected: GUE ~0.5996, Poisson ~0.3863", flush=True)
    print("", flush=True)
    ...
    print(
        f"  GUE blocks:     {np.mean(gue_r_means):.6f} +/- {np.std(gue_r_means):.6f} "
        f"(expected ~0.5996)",
        flush=True,
    )
    print(
        f"  Poisson blocks: {np.mean(poi_r_means):.6f} +/- {np.std(poi_r_means):.6f} "
        f"(expected ~0.3863)",
        flush=True,
    )
    ...
    print(
        f"  GUE blocks:     {np.mean(gue_entropies):.4f} +/- {np.std(gue_entropies):.4f}",
        flush=True,
    )
    print(
        f"  Poisson blocks: {np.mean(poi_entropies):.4f} +/- {np.std(poi_entropies):.4f}",
        flush=True,
    )

    print("\n  --- KS Margin (KS_poi - KS_gue; positive = closer to GUE) ---", flush=True)
    print(
        f"  GUE blocks:     {np.mean(gue_ks_margins):+.6f} +/- {np.std(gue_ks_margins):.6f}",
        flush=True,
    )
    print(
        f"  Poisson blocks: {np.mean(poi_ks_margins):+.6f} +/- {np.std(poi_ks_margins):.6f}",
        flush=True,
    )

    print("\n  --- Classification Accuracy ---", flush=True)
    print(
        f"  GUE -> GUE:       {gue_correct}/{len(gue_results)} ({100 * gue_accuracy:.1f}%)",
        flush=True,
    )
    print(
        f"  Poisson -> Poisson: {poi_correct}/{len(poi_results)} "
        f"({100 * poi_accuracy:.1f}%)",
        flush=True,
    )
    print(
        f"  Total accuracy:   {gue_correct + poi_correct}/{len(all_results)} "
        f"({100 * total_accuracy:.1f}%)",
        flush=True,
    )
    ...
    print(
        f"  {'block_id':<20} {'r_mean':>8} {'entropy':>8} "
        f"{'ks_gue':>8} {'ks_poi':>8} {'margin':>8} {'vote':>8}",
        flush=True,
    )
    print(
        f"  {'-' * 20} {'-' * 8} {'-' * 8} {'-' * 8} {'-' * 8} {'-' * 8} {'-' * 8}",
    ...
    for r in gue_results[:5]:
        print(
            f"  {r['block_id']:<20} {r.get('r_mean', 0):8.4f} {r.get('entropy', 0):8.4f} "
            f"{r.get('ks_gue_stat', 0):8.4f} {r.get('ks_poi_stat', 0):8.4f} "
            f"{r.get('ks_margin', 0):+8.4f} {r.get('vote', '?'):>8}",
            flush=True,
        )
    ...
    if aligned:
        print("  STATUS: DISCRIMINATOR_VALIDATED", flush=True)
        print(
            f"  The gap-ratio KS discriminator separates GUE from Poisson at "
            f"{100 * total_accuracy:.1f}% accuracy.",
            flush=True,
        )
        print(
            f"  When applied to real zeta blocks, if KS(r, GUE) < KS(r, Poi) "
            f"in > {100 * args.vote_threshold:.0f}% of blocks",
            flush=True,
        )
        print("  -> 'GUE alignment confirmed'.", flush=True)
    else:
        print("  STATUS: DISCRIMINATOR_WEAK", flush=True)
        print(
            "  Accuracy below threshold. Consider larger block_length "
