...
from controls import build_blocks_from_spacings
...
def _process_block(args: Tuple) -> Tuple[str, str, int, str, List[Dict], Dict]:
    ...
    """
    (block, run_id, flow_steps, flow_points, flow_float32, reducer_cfg, embed_dim) = args
...
def main():
    ...
    try:
        for iteration in range(start_iter, start_iter + max_iters):
            ...
            log(
                f"\n--- Iteration {iteration + 1}/{start_iter + max_iters} (seed={iter_seed}) ---",
                log_file,
            )
            t0 = time.time()
            ...
            log(
                f"  Total blocks: {len(all_blocks)} (gue={n_gue}, poisson={n_poi})",
                log_file,
            )
                ...
                with Pool(processes=min(n_cores, len(worker_args))) as pool:
                    for result in pool.imap_unordered(_process_block, worker_args):
                        ...
                        if processed % 5 == 0 or processed == len(all_blocks):
                            log(
                                f"    processed {processed}/{len(all_blocks)} "
                                f"({block_type}) rows={writer.total_rows}",
                                log_file,
                            )
            ...
            dt = time.time() - t0
            log(
                f"  Iteration time: {dt:.1f}s | merged: {writer.file_size_mb:.2f} MB | "
            ...
            for btype, s in report.get("summary_by_type", {}).items():
                log(
                    f"    {btype}: n={s['n_blocks']} converged={s['pct_converged']:.0f}% "
                    f"stalled={s['pct_stalled']:.0f}%",
                    log_file,
                )

            if report.get("top_outliers"):
                log("  TOP OUTLIERS:", log_file)
                for o in report["top_outliers"][:5]:
                    log(
                        f"    {o['block_id']}: score={o['outlier_score']:.4f} "
            ...
            if stagnation_count >= 2:
                ...
                log("  STOP: Metrics stagnant for 2 iterations", log_file)
                break
    ...
    print("  audit_report.json, outliers.csv, cycle_state.json", flush=True)
