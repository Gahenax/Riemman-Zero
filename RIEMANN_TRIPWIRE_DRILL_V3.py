...
class DrillConfig:
    ...
    split_ratio: float = (
        0.35  # si h > split_ratio * Delta(T) y hubo bracket, considerar split
    )
...
def drill_interval_recursive(
    ...
    rr = refine_root_bracket(
        f, mp.mpf(a), mp.mpf(b), fa, fb, tol=tol, max_iter=cfg.max_iter_root
    )
...
def drill_block(
    block_start: float, block_end: float, cfg: DrillConfig, out_path: str
) -> List[float]:
    mp.mp.dps = cfg.dps
    roots: List[float] = []
            ...
            while t < block_end:
                ...
                if z_prev == 0 or z_next == 0 or (z_prev * z_next) <= 0:
                    _ = drill_interval_recursive(
                        hardy_Z, t, t_next, z_prev, z_next, cfg, roots, fp, depth=0
                    )
...
def find_worst_block_from_jsonl(
    jsonl_path: str, default_block: float = 100.0
...
def main():
    ap = argparse.ArgumentParser(
        description="Riemann Drill v3: deficit-directed split-on-demand mining"
    )
    ap.add_argument(
        "--jsonl",
        type=str,
        default="",
        help="JSONL previo con block_summary (para elegir peor bloque)",
    )
