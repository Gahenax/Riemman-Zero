...
class LatticeOracle:
    ...
    def __init__(
        self, mix_factor: int = DEFAULT_MIX_FACTOR, mix_k_abs: int = DEFAULT_MIX_K_ABS
    ):
    ...
    def evaluate(self, challenge: LatticeChallenge, coeffs: np.ndarray) -> float:
...
def _gs_update_from(
    B: np.ndarray, mu: np.ndarray, Bstar: np.ndarray, Bstar_norms_sq: np.ndarray, start: int
) -> None:
...
def run_single_battle(
    ...
    ratio_lll = (
        norm_lll / challenge.gaussian_heuristic
        if challenge.gaussian_heuristic > 0
        else float("inf")
    )
    ...
    try:
        ...
        ratio_bb = (
            norm_bb / challenge.gaussian_heuristic
            if challenge.gaussian_heuristic > 0
            else float("inf")
        )
    ...
    for dim in dims:
        ...
        for s in range(args.samples):
            ...
            try:
                ...
                bb_ratio = (
                    f"{bb_row.ratio_gh:.4f}"
                    if bb_row and bb_row.ratio_gh < float("inf")
                    else "FAIL"
                )

                print(
                    f"  Batalla {battle_num}/{total_battles} "
                    f"(Seed {seed})... "
                ...
                print(
                    f"  Batalla {battle_num}/{total_battles} (Seed {seed})... ERROR: {ex}"
                )
