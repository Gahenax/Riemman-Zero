...
# ============================================================
def directed_purge(
    B: Any, U: Any, i: int, maxabs_cache: Any, ledger: UALedger, tries: int, is_object: bool
) -> Tuple[Any, Any, Any, bool, int]:
    """
    ...
    for _ in range(int(tries)):
        ...
        if (not is_object) and (not isinstance(B, list)):
            if _check_overflow_risk(
                int(maxabs_cache[i]), abs(q), int(maxabs_cache[best_j])
            ):
...
def deep_lll_v2(
    ...
    block_size: int,
    k_gs: int,
) -> Tuple[Any, Any, Any, bool, int, str]:
    """
        ...
        for j in range(k - 1, -1, -1):
            if abs(mu[k, j]) > 0.5000000001:
                ...
                if q != 0:
                    ...
                    if (not is_object) and (not isinstance(B, list)):
                        if _check_overflow_risk(
                            int(maxabs_cache[k]), abs(q), int(maxabs_cache[j])
                        ):
                            B, U, maxabs_cache = escalate_to_object(B, U, maxabs_cache)
                            is_object = True
...
def run_universe(
    ...
    do_chaos: bool,
    rng: np.random.Generator,
    h_config: HardeningConfig = HardeningConfig(),
) -> Tuple[Any, Any, UniverseResult]:
    ...
    while ledger.alive():
        ...
        if run_chaos:
            ...
            str_now = (
                chaos_strength
                if mode == GearMode.MICRO_EXPLORE
                else max(1, chaos_strength // 2)
            )
...
def ua_engine_v2(basis: np.ndarray, **kwargs) -> Tuple[np.ndarray, Dict]:
    ...
    for a in range(attempts_A):
        ...
        # Abort reclaim
        if res_a.stop_reason in [
            "plateau_abort",
            "entropy_fuse_tripped",
            "corruption_trap",
        ]:
            aborted += 1
    ...
    log = {
        ...
        **total_ledger.snapshot(),
        ...
        "deep_insert_exec_count": int(
            sum(r.deep_insert_count for _, _, r in phase_a_results)
        ),
        ...
        "kick_rate_A": float(
            np.mean([1.0 if r.kick_triggered else 0.0 for _, _, r in phase_a_results])
        )
        if phase_a_results
        else 0.0,
        "stop_reason": "budget",
...
def bench_runner():
    ...
    print("=" * 100)
    print(
        f"{'Dim':>4} {'Seed':>6} {'LLL norm':>12} {'UA norm':>12} "
    ...
    for dim in dims:
        for seed in seeds:
            ...
            if lll_reduce is not None:
                ...
                norms_lll = [
                    float(np.linalg.norm(B_lll[i].astype(np.float64))) for i in range(dim)
                ]
            ...
            print(
                f"{dim:>4} {seed:>6} {lll_norm:>12.4f} {ua_norm:>12.4f} "
    ...
    print("=" * 100)
    print(
        f"{'Dim':>4} {'LLL mean':>11} {'UA mean':>11} {'UA best':>11} "
    ...
    for dim in dims:
        ...
        print(
            f"{dim:>4} {np.mean(lll_r):>11.4f} {np.mean(ua_r):>11.4f} "
