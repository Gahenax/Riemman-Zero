...
def lattice_step_fn(
    A: np.ndarray, rng: np.random.Generator, cfg: HardeningConfig, sanity: SanityStats
) -> np.ndarray:
    """
    ...
    U = eye_like(n, B)
    maxabs = np.array(
        [_maxabs_row(B, i) for i in range(n)], dtype=object if is_object else np.int64
    )
    ecap = entropy_cap_compute(B)

    # Minimal chaos
    B, U, maxabs, is_object = chaos_mix_v2(
        B, U, maxabs, inner_ledger, is_object, 1, ecap, 2, rng
    )

    # Deep LLL (full breadth)
    B, U, maxabs, is_object, _, _ = deep_lll_v2(
        B, U, maxabs, inner_ledger, is_object, 0.99, 0.98, n, 10
    )
...
def ua_engine_v2_hardened(basis: np.ndarray, **kwargs) -> Tuple[np.ndarray, Dict]:
    ...
    log = {
        "n": n,
        "UA_spent": float(
            getattr(result.get("ua"), "spent", 0.0) if result.get("ua") else 0.0
        ),
