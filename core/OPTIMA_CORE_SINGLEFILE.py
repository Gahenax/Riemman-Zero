...
def gs_update_row_k(
    B: np.ndarray, mu: np.ndarray, Bstar: np.ndarray, Bstar_norm: np.ndarray, k: int
) -> None:
...
def pick_deep_insert(
    mu: np.ndarray, Bstar_norm: np.ndarray, delta: float, max_jump: int = 8
) -> Tuple[int, int, float]:
    """
...
def global_policy_solver_deepswap(
    basis: np.ndarray, delta: float = 0.99, max_actions_factor: int = 20, max_jump: int = 8
) -> np.ndarray:
    """
    ...
    while actions < max_actions:
        ...
        # 3) Deep insert move (break ties / reorder globally)
        src, dst, gain = pick_deep_insert(mu, Bstar_norm, delta=delta, max_jump=max_jump)
...
def random_unimodular_mix_U(
    n: int, strength: int = 2, rng: Optional[np.random.Generator] = None
) -> np.ndarray:
    """
...
def black_box_solver(basis: np.ndarray, **kwargs: Any) -> np.ndarray:
    ...
    return global_policy_solver_deepswap(
        basis=basis, delta=delta, max_actions_factor=max_actions_factor, max_jump=max_jump
    )
...
if __name__ == "__main__":
    ...
    print(
        "ALL GREEN"
        if n_bb <= n_lll + 1e-12
        else f"DeepSwap WORSE by {(n_bb / n_lll - 1) * 100:.2f}%"
    )
