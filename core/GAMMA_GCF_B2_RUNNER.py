...
class FutilityMonitor:
    def __init__(
        ...
        max_stale: int = FUTILITY_WINDOWS,
...
def run_b2_search(target: mp.mpf, time_limit: float = 900.0):
    ...
    P(
        f"[CONFIG] Stages: S0(N={N0},G0={G0}) -> S1(N={N1}/{N2},G1={G1}) -> S2(N={N2_DEEP}/{N2_DEEPER},Smin={SMIN})"
    )
    P(f"[CONFIG] Anti-cluster: B={BUCKET_DECIMALS} decimals, M={BUCKET_MAX}/bucket")
    P(
        f"[CONFIG] Futility: W={WINDOW_SIZE}, {FUTILITY_WINDOWS} stale windows, {FUTILITY_FACTOR}x factor"
    )
    P("")
    ...
    for q_poly in q_safe:
        for p_poly in p_space:
            ...
            if stats.total_checked % hb == 0:
                ...
                P(
                    f"  [HB] {stats.total_checked:,}/{total_pairs:,} | {rate:,.0f}/s | "
            ...
            # ==================== STAGE 1 ====================
            try:
                eval_gcf(p_poly, q_poly, N1, PREC0_DPS)
            ...
            if gap2 < stats.overall_best_gap:
                ...
                P(
                    f"  *** NEW BEST GAP: {gap2:.4e} | a={poly_to_str(p_poly)} b={poly_to_str(q_poly)}"
                )
    ...
    all_entries.sort(
        key=lambda e: (e["gap_float"], -e["stability_score"], e["complexity_cost"])
    )
...
def print_report(
    ...
    P(
        f"  Futility: W={WINDOW_SIZE}, max_stale={FUTILITY_WINDOWS}, factor={FUTILITY_FACTOR}"
    )
    ...
    if not top_k:
        ...
        for i, c in enumerate(top_k):
            label = (
                ...
                if c["gap_float"] <= 1e-20 and c["stability_score"] >= SMIN
                else "SUB-THRESHOLD"
            )
            P(f"\n  {label} #{i + 1}")
...
def main():
    ...
    data = {
        ...
        "parameters": {
            ...
            "futility_windows": FUTILITY_WINDOWS,
        ...
        "stats": {
            ...
            "s0_pass": stats.s0_pass,
            "s0_fail": stats.s0_fail,
            "s1_pass": stats.s1_pass,
            "s1_fail": stats.s1_fail,
            "s2_pass": stats.s2_pass,
            "s2_fail": stats.s2_fail,
