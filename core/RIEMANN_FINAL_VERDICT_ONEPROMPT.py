...
def mean_control_K(
    kind: str, block_size: int, taus: np.ndarray, seeds: List[int], trim: float = 0.10
) -> np.ndarray:
    Ks = []
    for sd in seeds:
        if kind.upper() == "GUE":
...
def generate_controls(
    block_size: int, taus: np.ndarray, seeds: List[int], trim: float = 0.10
) -> Dict[str, Any]:
    print(
        f"[CONTROLS] Generating GUE ({len(seeds)} seeds, N_in~{int(1.35 * block_size)})...",
        flush=True,
    )
...
def make_blocks(
    zeros: np.ndarray, block_size: int, mode: str = "disjoint"
) -> List[np.ndarray]:
    ...
    return [zeros[i : i + block_size] for i in range(0, zeros.size - block_size + 1, step)]
...
def audit_blocks(
    ...
    for idx, zb in enumerate(blocks):
        ...
        metrics.append(m)
        print(
            f"  block {idx}: gr={m['gap_ratio']:.4f} beta={m['beta_mle']:.3f} "
            f"d_gue={m['d_gue']:.3f} d_poi={m['d_poi']:.3f} => {m['vote']}",
            flush=True,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--block_mode", type=str, default="disjoint", choices=["disjoint", "overlap50"]
    )
    ...
    p.add_argument("--control_seeds", type=str, default="41,42,43,44,45,46,47,48")
...
def main() -> int:
    ...
    print(f"\n{'+' + '=' * 68 + '+'}", flush=True)
    print(
        "|  RIEMANN FINAL VERDICT -- GUE Universality Audit                 |", flush=True
    )
    print(
    ...
    controls = generate_controls(args.block_size, taus, seeds=seeds, trim=args.gue_trim)
    ...
    print(f"\n{'=' * 80}", flush=True)
    print("FINAL AUDIT VERDICT", flush=True)
    print(f"{'=' * 80}", flush=True)
    print(f"  Verdict:        {report['verdict']}", flush=True)
    print(f"  GUE vote rate:  {report['gue_vote_rate']:.2%}", flush=True)
    s = report["stats"]
    print(
        f"  Gap Ratio:      {s['gap_ratio']['mean']:.4f} "
        f"CI95=[{s['gap_ratio']['CI95'][0]:.4f}, {s['gap_ratio']['CI95'][1]:.4f}] "
        f"(GUE~0.60, Poi~0.39)",
        flush=True,
    )
    print(
        f"  Beta MLE:       {s['beta_mle']['mean']:.3f} "
        f"CI95=[{s['beta_mle']['CI95'][0]:.3f}, {s['beta_mle']['CI95'][1]:.3f}] "
        f"(GUE~2.0, Poi~0.0)",
        flush=True,
    )
    print(
        f"  d(R,GUE):       {s['dist_gue']['mean']:.4f} "
        f"CI95=[{s['dist_gue']['CI95'][0]:.4f}, {s['dist_gue']['CI95'][1]:.4f}]",
        flush=True,
    )
    print(
        f"  d(R,Poisson):   {s['dist_poi']['mean']:.4f} "
        f"CI95=[{s['dist_poi']['CI95'][0]:.4f}, {s['dist_poi']['CI95'][1]:.4f}]",
        flush=True,
    )
    print("", flush=True)
    ...
    if report.get("verdict") == "CONFIRMED_GUE_UNIVERSALITY":
        print(
            "[SYSTEM] PASSED: Riemann zeros estadisticamente indistinguibles "
            "de universalidad GUE (bajo este pipeline).",
            flush=True,
        )
        print("", flush=True)
    ...
    print(
        "[SYSTEM] INCONCLUSIVE: increase total_zeros, adjust block_size, or use overlap50.",
        flush=True,
    )
    print("", flush=True)
    print("DISCLAIMER: Statistical compatibility != proof of RH.", flush=True)
