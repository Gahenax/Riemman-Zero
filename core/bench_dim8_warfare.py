...
def example_step_fn(
    A: np.ndarray, rng: np.random.Generator, cfg: HardeningConfig, sanity
) -> np.ndarray:
...
def run_benchmark(
    ...
    for dim in dims:
        for seed in seeds:
            ...
            row = {
                ...
                "ours_ua_spent": float(
                    getattr(ours.get("ua"), "spent", ours.get("ua", {}).get("spent", 0.0))
                    if ours.get("ua") is not None
                    else 0.0
                ),
                "ours_ua_refund": float(
                    getattr(ours.get("ua"), "refund", ours.get("ua", {}).get("refund", 0.0))
                    if ours.get("ua") is not None
                    else 0.0
                ),
    ...
    print(
        f"runs={len(rows)} dims={dims} ua_budget={ua_budget} iter_max={iter_max} wall_seconds={dt:.2f}"
    )
