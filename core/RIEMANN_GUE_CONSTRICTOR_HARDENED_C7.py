...
def pair_correlation(
    spacings: np.ndarray, r_max: float = 3.0, n_bins: int = 50
...
def number_variance_empirical(positions: np.ndarray, L: float, step: float = 0.25) -> float:
    """
...
def delta3_empirical(positions: np.ndarray, L: float, step: float = 0.25) -> float:
    """
...
def run_window(n_start: int, count: int, config: dict) -> dict:
    ...
    c1_pass = (
        abs(beta - thresholds["beta_target"]) <= thresholds["beta_tol"]
    ...
    all_pass = (
        c1_pass and c2_pass and c3_pass and c5_pass and c6_pass and c7_s_pass and c7_d_pass
    )
...
def main():
    ...
    ap.add_argument(
        "--windows", type=str, default=None, help="Comma-separated n_start list"
    )
    ...
    gate_b = (
        (1.8 <= avg_beta <= 2.2 if not np.isnan(avg_beta) else False)
        and avg_ks <= 0.05
        and avg_d3 <= 0.20
    )
    ...
    P("")
    P("DISCLAIMER: GUE compatibility is a statistical observation, NOT a proof of RH.")
    P("All results are reproducible with the provided config and seed.")
    P("")
