...
class GearboxController:
    ...
    def should_shift(self) -> Tuple[bool, GearMode, str]:
        ...
        if (
            norm_improved
            and grad_now < self.cfg.grad_min
            and geom_now >= self.cfg.geom_flat_min
        ):
...
class ShortTermActionMemory:
    ...
    def suggest_action(
        self, metrics: StepMetrics, candidates: List[str], fallback: str
    ) -> str:
        """
...
class RiemannConfidenceScorer:
    ...
    def score(self, det: RiemannDetection) -> float:
        ...
        if width <= self.cfg.width_good:
            ...
            s_width = 1.0 - (width - self.cfg.width_good) / (
                self.cfg.width_bad - self.cfg.width_good
            )
        ...
        if det.precision_bits >= self.cfg.precision_good:
            ...
            s_prec = (det.precision_bits - self.cfg.precision_bad) / (
                self.cfg.precision_good - self.cfg.precision_bad
            )
        ...
        # Repeat score (satura)
        s_repeat = clamp01(
            math.log1p(det.repeats) / math.log1p(8.0)
        )  # 8 repeticiones ≈ 1.0
...
class OutputLogger:
    ...
    def log(self, state: OpState, msg: str, metrics: Optional[StepMetrics] = None) -> None:
        if state == OpState.SILENT:
            ...
            print(
                f"[SILENT] step={metrics.step} norm={metrics.norm:.6g} grad={metrics.grad_eff:.3g} geom={metrics.geom_flat:.3g} swaps={metrics.swaps}"
            )
            return
        print(msg)
...
def demo():
    ...
    governor = GlobalGovernor(
        GovernorConfig(silent_on_stagnation=True, silent_duration_steps=120)
    )
