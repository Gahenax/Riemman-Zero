...
def objective_J(B: np.ndarray, lambda1: float = 0.02, lambda2: float = 0.02) -> float:
    ...
    return float(
        np.min(norms_sq) + lambda1 * np.median(log_norms) + lambda2 * np.median(log_max)
    )
...
def deep_lll(
    B: np.ndarray, U: np.ndarray, ledger: UALedger, delta: float = 0.99, alpha: float = 0.9
) -> None:
    """
...
if __name__ == "__main__":
    ...
    for dim in [10, 15, 20]:
        ...
        t0 = time.time()
        coeffs = black_box_solver(
            basis, delta=0.99, ua_budget=dim * dim * 80, attempts=5, seed=42
        )
        ...
        print(
            f"  dim={dim:>3}  ||v||={norm:>12.4f}  ||v||²_exact={ns_exact:>14}  "
