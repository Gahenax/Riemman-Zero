...
def zero_isolation_reset(
    A: np.ndarray, idx: int, rng: np.random.Generator, eps_norm: float
) -> np.ndarray:
    """
...
def mark_seed_fail(
    scores: Dict[int, SeedScore], seed: int, iter_i: int, cfg: HardeningConfig, mode: str
) -> None:
