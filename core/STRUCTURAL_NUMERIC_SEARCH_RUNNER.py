...
class EntropyReducerScanner:
    ...
    def scan_target(
        self, name: str, target: mpf
    ) -> Tuple[List[FullCandidate], EntropyReport]:
        er = EntropyReport()
        ...
        canon_before = (
            len(
                [
                    p
                    for p in itertools.product(COEFF_RANGE, repeat=MAX_DEGREE + 1)
        ...
        for a_poly in self.poly_space:
            for b_poly in self.poly_space:
                ...
                if checked % hb == 0:
                    ...
                    P(
                        f"  [HB] {checked:,}/{self.total_pairs:,} | {rate:,.0f}/s | cands={len(candidates)}"
                    )
        ...
        er.add_step(
            ...
            self.total_pairs,
            survived_prefilter,
            f"Rejected {total_div:,} ({div_detail})",
        )
        er.add_step(
            "Ballpark gating (2-depth)",
            survived_prefilter,
            survived_ballpark,
            f"Rejected {ballpark_rejected:,}",
        )
        er.add_step(
            "Stability triage (N,2N)",
            survived_ballpark,
            survived_triage,
            f"Rejected {triage_rejected:,}",
        )
        er.add_step(
            "Full multi-depth + tolerance",
            survived_triage,
            len(candidates),
            f"Depth rej={depth_rejected}, tol rej={tol_rejected}",
        )
...
def print_candidate(c: FullCandidate, compute_time: float):
    ...
    P(
        f"  a complexity:     L1={c.a_complexity[0]} nz={c.a_complexity[1]} |b0|={c.a_complexity[2]}"
    )
    P(f"b(n):               {c.b_poly}  ->  {c.b_str}")
    P(
        f"  b complexity:     L1={c.b_complexity[0]} nz={c.b_complexity[1]} |b0|={c.b_complexity[2]}"
    )
    P(f"Depths:             {c.depths}")
    P(f"Values:             {c.values}")
...
def main():
    ...
    P("Reducer:      Canonicalization + Divergence + Ballpark(2-depth) + Triage(N,2N)")
    P("Ordering:     Complexity prior (L1 norm ascending)")
    P("")
    ...
    P(
        f"  B) Tail sensitivity: perturb last {TAIL_K} terms by {TAIL_PERTURB}, delta < {TAIL_THRESHOLD}"
    )
    P(f"  C) Cross-check: err_target / err_shifted ratio > 5 (eps={CROSS_EPS})")
    P("  D) Calibration: run phi, 4/pi first to validate pipeline")
    P("")
    ...
    if total_accepted == 0:
        ...
        P(
            f"{total_accepted} candidate(s) survive as NUMERICAL COINCIDENCES for human review."
        )
