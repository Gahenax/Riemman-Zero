...
from hardening import (
    ...
    condition_proxy,
    find_bad_rows,
    zero_isolation_reset,
    fuse_should_abort,
    stagnation_should_abort,
    ua_refund_on_abort,
    seed_is_toxic,
    mark_seed_fail,
...
StepFn = Callable[
    [np.ndarray, np.random.Generator, HardeningConfig, SanityStats], np.ndarray
]
...
def run_universe_hardened(
    ...
    for t in range(iter_max):
        ...
        fuse.norm_log_hist.append(
            float(np.log(np.linalg.norm(A, ord="fro") + cfg.eps_norm))
        )
...
def run_with_retries(
    ...
    """
    out = run_universe_hardened(
        seed, A_init, step_fn, score_fn, ua_budget, iter_max, cfg, seed_scores, global_iter0
    )
