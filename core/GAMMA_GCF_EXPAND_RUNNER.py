...
def complexity_cost(p: Poly, q: Poly) -> int:
    return (
        sum(abs(c) for c in p)
        + sum(abs(c) for c in q)
        + 2 * poly_degree(p)
        + 2 * poly_degree(q)
    )
...
def build_poly_spaces(max_degree: int, coeff_range: range):
    ...
    return sorted(p_set, key=lambda x: sum(abs(c) for c in x)), sorted(
        q_set, key=lambda x: sum(abs(c) for c in x)
    )
...
def run_phase(
    ...
    P(
        f"[PRUNE] Q singularity filter: {len(q_space)} -> {len(q_ok)} (rejected {q_sing_rej})"
    )
    ...
    for q_poly in q_ok:
        for p_poly in p_space:
            ...
            if checked % hb == 0:
                ...
                P(
                    f"  [HB] {checked:,}/{effective_pairs:,} | {rate:,.0f}/s | eval={evaluated} | stab={len(results)}"
                )

            if time.time() - t0 > time_limit:
                time_limited = True
                P(
                    f"  [TIME LIMIT] {time_limit:.0f}s reached at {checked:,}/{effective_pairs:,}"
                )
                break
    ...
    P(
        f"         evaluated={evaluated} | stable(score>=12)={stable_count} | top_k={len(top_k)}"
    )
    P(
        f"         best_gap={mp.nstr(best_gap, 10)} | best_stab={best_stab:.1f} | best_cost={best_cost}"
    )
...
def print_report(results: List[ScanResult]):
    ...
    for r in results:
        ...
        P(
            f"  Pruning: Q_sing={r.q_singular_rejected}, ratio={r.ratio_rejected:,}, fast={r.fast_rejected:,}"
        )
        ...
        if r.candidates:
            for i, c in enumerate(r.candidates):
                ...
                P(f"    gap:          {c['gap']}")
                P(
                    f"    VERDICT:      {'CANDIDATE' if c['passed'] else 'REJECTED (sub-threshold)'}"
                )
                P("")
                if c["passed"]:
    ...
    if total_candidates_passed > 0:
        P(
            f"gamma: SIG_CANDIDATES ({total_candidates_passed} candidate(s) with StabilityScore >= {STABILITY_SCORE_MIN})"
        )
        P(
            "WARNING: This is NOT proof of a closed form. Numerical stability does not imply identity."
        )
    else:
        P("gamma: NO_SIG")
        ...
        for r in results:
            P(
                f"  {r.phase}: best_gap={r.best_gap}, best_stab={r.best_stability:.1f}, best_cost={r.best_cost}"
            )
    P("")
...
def main():
    ...
    P("Precision source: hardcoded 96-digit string")
