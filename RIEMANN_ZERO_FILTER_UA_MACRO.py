...
# Local imports
from core.gahenax_utils import UALedger
from core.GAHENAX_UPGRADES import (
    RiemannDetection,
    RiemannConfidenceConfig,
    RiemannConfidenceScorer,
)
...
class RiemannLedger(UALedger):
    ...
    def can_spend_A(self, ua: int) -> bool:
        return (self.A_spent + ua) <= self.A_budget

    def can_spend_B(self, ua: int) -> bool:
        return (self.B_spent + ua) <= self.B_budget

    def spend_A(self, ua: int):
        if not self.can_spend_A(ua):
            raise RuntimeError("A budget exhausted")
        self.A_spent += ua
        self.ua_flow += ua
        self.pay("mix", ua)

    def spend_B(self, ua: int):
        if not self.can_spend_B(ua):
            raise RuntimeError("B budget exhausted")
        self.B_spent += ua
        self.ua_kick += ua
        ...
        if z_prev * z_curr <= 0:
            ...
            if ledger.can_spend_B(ua_cost_brent):
                ...
                try:
                    ...
                    def f(x):
                        return hardy_z(float(x), cfg_prec.dps_high)
...
def main():
    ...
    cfg_prec = PrecisionConfig(
        dps_low=args.dps_low, dps_mid=args.dps_mid, dps_high=args.dps_high
    )
