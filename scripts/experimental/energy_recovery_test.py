...
def compute_metrics(
    ...
    energy_far = (
        float(np.mean(dpsi[~mask_near] ** 2)) if (~mask_near).any() else float("nan")
    )
...
def weights_mode(
    gammas: np.ndarray, weights_hat: Optional[np.ndarray], mode: str
) -> np.ndarray:
...
def main() -> int:
    ...
    for mode in args.modes:
        ...
        print(
            f"[*] mode={mode:8}  ratio={metrics['ratio_near_far']:.4f}  hit={metrics['hit_rate_topK']:.4f}"
        )
    ...
    print(
        f"[*] Wrote summary: {os.path.join(args.out_dir, 'summary_energy_recovery.json')}"
    )
    return 0
