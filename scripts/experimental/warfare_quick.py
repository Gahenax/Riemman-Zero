...
def run_quick_warfare(dim=20, seeds=[1000]):
    ...
    print("=" * 90)
    print(
        f"{'Seed':>6} {'LLL norm':>12} {'UA norm':>12} {'Result':>8} {'UA_spent':>8} {'Status':>12} {'t(s)':>7}"
    )
    ...
    for seed in seeds:
        ...
        t0 = time.time()
        coeffs, log = ua_engine_v2(
            basis, ua_budget=dim * dim * 500, iter_max=500, seed=seed
        )
        t1 = time.time()
        ...
        verdict = (
            ...
            if abs(ua_norm - lll_norm) < 1e-6
            else "LOSE"
        )

        print(
            f"{seed:>6} {lll_norm:>12.4f} {ua_norm:>12.4f} {verdict:>8} {log['UA_spent']:>8.0f} {log['status']:>12} {t1 - t0:>7.2f}"
        )
