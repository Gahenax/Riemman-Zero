...
def bin_deficits(
    zeros_sorted: np.ndarray, T0: float, T1: float, bin_width: float
) -> List[BinDeficit]:
    z = np.asarray(zeros_sorted, dtype=float)
    z.sort()
    ...
    for i in range(len(bins) - 1):
        ...
        out.append(
            BinDeficit(a=a, b=b, expected=exp, found=found, deficit=max(0.0, exp - found))
        )
    return out
...
class ZEngine:
    ...
    def drill_focus(
        self, a: float, b: float, alpha: float, known_zeros: List[float]
    ) -> List[float]:
...
def run_pipeline(cfg: RunConfig, hooks: Hooks) -> Dict:
    ...
    completeness = (
        float(found_total / expected_total) if expected_total > 0 else float("nan")
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Antigravity Lab: Rescue Miner (Explore/Focus/Verify) + mpmath Hardy Z."
    )
    ...
    p.add_argument(
        "--no_zeros_in_json",
        action="store_true",
        help="No incluir la lista completa de ceros en el JSON.",
    )
    return p.parse_args()
        ...
        json.dumps(
            {
                ...
                "deficits_top_0": report["deficits_top"][0]
                if report["deficits_top"]
                else None,
            },
            ensure_ascii=False,
            indent=2,
