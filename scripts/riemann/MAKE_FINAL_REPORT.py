...
def main():
    ...
    if not all(
        os.path.exists(p) for p in [riemann_stats_path, cronos_grid_path, alignment_path]
    ):
    ...
    a1 = any(p["r_mean"] < 0.45 for p in cronos_points) and any(
        p["r_mean"] > 0.57 for p in cronos_points
    )
