...
def refine_root_bracket(
    f, a: mp.mpf, b: mp.mpf, fa: mp.mpf, fb: mp.mpf, tol: mp.mpf, max_iter: int = 80
) -> RootResult:
    """
            ...
            while t < bend:
                ...
                if z_prev == 0 or z_next == 0 or (z_prev * z_next) <= 0:
                    ...
                    rr = refine_root_bracket(
                        hardy_Z, a, b, fa, fb, tol=tol, max_iter=max_iter_root
                    )

                    t_root = float(rr.root)
                    ok = rr.converged and (
                        abs(float(rr.f_at_root)) <= float(mp.mpf(10) * tol)
                    )
                    inserted = False
                    if ok:
...
def main():
    ap = argparse.ArgumentParser(
        description="Riemann Tripwire Miner v2 (sign-change bracketing + robust refine)"
    )
