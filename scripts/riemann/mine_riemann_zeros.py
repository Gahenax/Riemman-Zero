...
def process_block(t1: float, t2: float, alpha: float = 0.25, budget: int = 500000):
    ...
    print(
        f"[ORCHESTRATOR] Block Stats: Expected={expected}, Observed={observed}, Deficit={deficit}"
    )
...
def main():
    ...
    while current_t < END_T:
        ...
        if deficit > DEFICIT_THRESHOLD:
            print(
                f"[CORRECTION] Deficit {deficit} > {DEFICIT_THRESHOLD}. Tightening alpha for next block."
            )
            alpha = max(0.05, alpha * 0.7)  # Reduce step size
        elif deficit <= 0 and alpha < DEFAULT_ALPHA:
            print("[CORRECTION] Zero deficit. Slightly relaxing alpha.")
    ...
    print("\n" + "=" * 60)
    print("INDUSTRIAL MINING COMPLETE.")
