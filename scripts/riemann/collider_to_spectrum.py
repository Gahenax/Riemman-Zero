...
def filter_signal(
    events: List[Dict[str, Any]], require_accepted: bool = True
) -> List[Dict[str, Any]]:
    out = []
    for e in events:
...
def find_peaks(y: np.ndarray, min_prominence: float, min_distance: int) -> List[int]:
    ...
    for i in candidates:
        left_min = (
            np.min(y[max(0, i - min_distance) : i])
            if i - min_distance >= 0
            else np.min(y[:i])
        )
        right_min = (
            np.min(y[i + 1 : i + 1 + min_distance])
            if i + 1 + min_distance <= n
            else np.min(y[i + 1 :])
        )
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.10,
        help="KDE bandwidth (should match width/smear scale)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=5.0,
        help="Depends on event count; tune once.",
    )
    ap.add_argument(
        "--min_distance", type=float, default=0.10, help="In mass units (GeV-like)."
    )
    ...
    print(
        f"[*] Mass range: [{meta['mmin']:.3f}, {meta['mmax']:.3f}]  events_used={meta['num_events_used']}"
    )
