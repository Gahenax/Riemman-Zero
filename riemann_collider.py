...
def sample_powerlaw_m(
    rng: np.random.Generator, m_min: float, m_max: float, n: float
) -> float:
...
def generate_collider_event(
    rng: np.random.Generator, cfg: ColliderConfig
) -> Tuple[float, str]:
...
def main():
    ...
    if len(cfg.riemann_gammas) > 0:
        cfg.riemann_weights = make_gamma_weights(
            cfg.riemann_gammas, mode=cfg.riemann_weight_mode
        )
    ...
    for i in range(min(10000, len(masses))):
        events_out.append({"m": masses[i], "kind": 1 if kinds[i] == "signal" else 0})
    ...
    if args.plot and HAS_MATPLOTLIB:
        ...
        plt.hist(
            m_arr, bins=200, range=(0, 500), histtype="step", label="Total", color="black"
        )
        ...
        if len(sig_m) > 0:
            plt.hist(
                ...
                histtype="stepfilled",
                alpha=0.3,
                label="Riemann Signal",
                color="blue",
            )

        plt.yscale("log")
        plt.title(
            f"Riemann Collider Invariant Mass Spectrum (N={cfg.n_events}, w={cfg.width})"
        )
